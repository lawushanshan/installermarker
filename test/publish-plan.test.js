...
test("creates a plan-only publish plan from verified release evidence", () => {
  const plan = createPublishPlan(verification(), releaseVerification(), { releaseTag: "v1.0.0", generatedAt: "2026-01-01T00:00:01.000Z" });
  ...
  assert.equal(plan.supplementalAssets.some((asset) => asset.name === "release-verification.json"), true);
  assert.match(formatPublishPlan(plan, "text"), /does not upload artifacts/);
});
...
test("rejects publish plans when release evidence is incomplete", () => {
  ...
  assert.throws(() => createPublishPlan(verification(), report, { releaseTag: "v1.0.0" }), /artifact-scan/);
});

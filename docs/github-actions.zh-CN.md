...
该工作流会下载两组 artifact，运行 `publish-plan`，并把 `publish-plan.json` 作为 `publish-plan-<run_id>` 上传。该命令会重新验证本地产物目录，校验最终发布证据契约，检查证据是否属于同一来源和清单，并记录发布审核时必须保持不变的安装包和补充证据文件名及 SHA-256 哈希。

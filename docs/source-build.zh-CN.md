...
使用 `build-native` 时，将 `build.strategy` 设为 `go-native`、`rust-native` 或 `python-native`，保留 `packaging: build-native`，并配置能在 `artifactDirectories` 中生成平台安装包的命令。Python 默认产物目录为 `dist` 和 `build`，当它在标准清单中识别到 Briefcase、PyInstaller、Nuitka 或 cx_Freeze 时，可能会给出 `build.suggestedPackager` 和 `build.suggestedCommand`，但入口和最终命令仍然需要人工审核。审核人必须针对固定 commit 选择并审核该命令。Worker 刻意不替审核人选择打包器：Windows、macOS 和 Linux 的打包与签名要求不同。

...
The workspace must not already exist. InstallerMarker clones the public GitHub source at the reviewed commit, disables system Git configuration and hooks before cloning, uses an isolated home and npm configuration, runs the reviewed command, collects only platform-compatible installer files, hashes them, and writes `build-artifacts.json`. If the reviewed build command emits common SBOM document filenames such as SPDX or CycloneDX JSON files inside the configured artifact directories, the worker copies those documents to the output directory and records their format, source path, size, and SHA-256 as build SBOM evidence. It does not generate or enrich those SBOM documents itself. The manifest records source, build command, artifact hashes, optional build-produced SBOM document hashes, start and finish timestamps, duration, and the Node.js runner platform/architecture used for the build. Artifact directories must be real directories inside the checkout, not symbolic links; each installer is limited to one GiB and each SBOM document is limited to 20 MiB. It never overwrites an existing output file. The checkout remains available for build-log inspection; delete it only after reviewing the result.
